...
**重要提示：在开始之前请阅读整个文档。** 本文档涵盖了关键的 XML 架构规则和格式要求。错误的实现可能创建 PowerPoint 无法打开的无效 PPTX 文件。

## 技术指南

### 架构合规性
- **`<p:txBody>` 中的元素顺序**：`<a:bodyPr>`、`<a:lstStyle>`、`<a:p>`
- **空格**：在带有前导/尾随空格的 `<a:t>` 元素中添加 `xml:space='preserve'`
...
<p:sp>
  <p:nvSpPr>
    <p:cNvPr id="2" name="Title"/>  <!-- 名称保留为英文以便识别 -->
    <p:cNvSpPr>
      <a:spLocks noGrp="1"/>
    </p:cNvSpPr>
    <p:nvPr>
...
<p:graphicFrame>
  ...
  <a:graphic>
    <a:graphicData uri="http://schemas.openxmlformats.org/drawingmltable">
      /2006/<a:tbl>
        <a:tblGrid>
          <a:gridCol w="3000000"/>
          <a:gridCol w="3000000"/>
...
注意：不要对剩余幻灯片重新编号 - 保持它们的原始 ID 和文件名。
