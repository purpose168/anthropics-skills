...
def recalc(filename, timeout=30):
    ...
    # 处理Linux和macOS之间超时命令的差异
    if platform.system() != 'Windows':
        timeout_cmd = 'timeout' if platform.system() == 'Linux' else None
        if platform.system() == 'Darwin':
            # 检查macOS上是否可用gtimeout
            try:
    ...
    if result.returncode != 0 and result.returncode != 124:  # 124是超时的退出码
        error_msg = result.stderr or '重算过程中发生未知错误'
        if 'Module1' in error_msg or 'RecalculateAndSave' not in error_msg:
            return {'error': 'LibreOffice宏未正确配置'}
        else:
            return {'error': error_msg}
    
    # 检查重算后的文件中的Excel错误 - 扫描所有单元格
    try:
        ...
        for err_type, locations in error_details.items():
            if locations:
                result['error_summary'][err_type] = {
                    'count': len(locations),
                    'locations': locations[:20]  # 最多显示20个位置
                }
